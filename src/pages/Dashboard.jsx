...
function Dashboard() {
    ...
    const handleClearDone = async () => {
        const doneInquiries = inquiries.filter(inq => inq.status === 'done');
        if (doneInquiries.length === 0) {
            showToast('No completed inquiries to clear.');
            return;
        }
        try {
            const ids = doneInquiries.map(inq => inq.id);
            ...
            showToast(`Cleared ${doneInquiries.length} completed inquiries.`);
            setShowConfirmClear(false);
        } catch (error) {
            console.error('Error clearing done inquiries:', error);
            showToast('Failed to clear. Check Supabase DELETE policy.');
        }
    };
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
                        <button onClick={() => navigate('/')} style={btnGhost}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M3 9L12 2L21 9V20C21 20.5304 20.7893 21.0391 20.4142 21.4142C20.0391 21.7893 19.5304 22 19 22H5C4.46957 22 3.96086 21.7893 3.58579 21.4142C3.21071 21.0391 3 20.5304 3 20V9Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M9 22V12H15V22" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                            Home
                        </button>
                        ...
                        <button onClick={() => setShowSettings(true)} style={btnPrimary}>
                ...
                <div style={{ marginTop: '2.5rem', paddingTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.5rem' }}>
                            <h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: '800', color: '#F5F3E7' }}>📦 Content Management</h2>
                            <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.3)', fontWeight: '500' }}>Manage website content live</span>
                        </div>
                        <ContentManager />
                    </motion.div>
                </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                <h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: '800', color: '#F5F3E7' }}>⚙️ Settings</h2>
                                <button 
                                    onClick={() => setShowSettings(false)}
                            ...
                            {/* --- Change Password --- */}
                            <div style={{ marginBottom: '1.5rem', padding: '1.2rem', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                                <h3 style={{ margin: '0 0 1rem', fontSize: '0.9rem', color: '#F0B90B', fontWeight: '700' }}>🔑 Change Password</h3>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <input
                                        ...
                                        style={{ padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.06)', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E8E6DF', fontSize: '0.88rem', outline: 'none' }}
                                    />
                                    <input
                                        type="password"
                                    ...
                                    <input
                                        ...
                                        style={{ padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.06)', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E8E6DF', fontSize: '0.88rem', outline: 'none' }}
                                    />
                                    <button onClick={handleChangePassword} style={{ ...btnPrimary, justifyContent: 'center', marginTop: '0.3rem' }}>
                                        Update Password
                                    </button>
                                </div>
                            </div>
                            ...
                            {/* --- Data Management --- */}
                            <div style={{ marginBottom: '1.5rem', padding: '1.2rem', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                                <h3 style={{ margin: '0 0 0.8rem', fontSize: '0.9rem', color: '#EF4444', fontWeight: '700' }}>🗑️ Data Management</h3>
                                <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.5)', margin: '0 0 0.8rem' }}>
                                    Remove all inquiries marked as "Done" from the database.
                                </p>
                                <button onClick={() => setShowConfirmClear(true)} style={{ 
                                    ...
                                    Clear {inquiries.filter(i => i.status === 'done').length} Completed Inquiries
                                </button>
                                    ...
                                    {showConfirmClear && (
                                        ...
                                        >
                                            <div style={{
                                                ...
                                                <p style={{ margin: '0 0 0.3rem', fontSize: '1.1rem' }}>⚠️</p>
                                                <p style={{ margin: '0 0 0.6rem', fontSize: '0.85rem', color: '#EF4444', fontWeight: '700' }}>
                                                    Are you sure?
                                                </p>
                                                <p style={{ margin: '0 0 1rem', fontSize: '0.78rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
                                                    This will permanently delete <strong style={{ color: '#EF4444' }}>{inquiries.filter(i => i.status === 'done').length}</strong> completed inquiries from the database. This action cannot be undone.
                                                </p>
                                                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                                                    <button onClick={() => setShowConfirmClear(false)} style={{ ...btnGhost, fontSize: '0.78rem', padding: '7px 18px' }}>
                                                        Cancel
                                                    </button>
                            ...
                            <div style={{ padding: '1.2rem', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                                <h3 style={{ margin: '0 0 1rem', fontSize: '0.9rem', color: '#F5F3E7', fontWeight: '700' }}>🎛️ Preferences</h3>
                                
                                {/* Toggle items */}
                                {[
                                    { label: 'Email Notifications', desc: 'Get notified on new inquiries', icon: '📧' },
                                    { label: 'Auto-Refresh Dashboard', desc: 'Refresh data every 30 seconds', icon: '🔄' },
                                    { label: 'Compact Card View', desc: 'Show smaller inquiry cards', icon: '📋' }
                                ].map((pref) => (
                                    <div key={pref.label} style={{ 
                                        ...
                                        <div>
                                            <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#E8E6DF' }}>{pref.icon} {pref.label}</div>
                                            <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.35)', marginTop: '2px' }}>{pref.desc}</div>
                                        </div>
                                        <button 
                                            onClick={() => showToast(`${pref.label}: Coming Soon!`)}
                                            style={{
                                                width: '42px',
                                                height: '24px',
                                                borderRadius: '12px',
                                ...
                                <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.25)', margin: '1rem 0 0', textAlign: 'center', fontStyle: 'italic' }}>
                                    More preferences coming in future updates
                                </p>
                            </div>
                        </motion.div>
                    </motion.div>
